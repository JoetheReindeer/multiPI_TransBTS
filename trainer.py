...
class Trainer(object):
    def __init__(self, args, model):
        ...
        test_img_paths = glob.glob("./data/test_data/*")
        test_gt = glob.glob("./data/test_gt/*")
        ...
        test_dataset = get_segmentation_dataset('new', img_paths=test_img_paths, mask_paths=test_gt)
    ...
    def training(self, epoch):
        ...
        for iteration, sample in enumerate(tbar):
            ...
            target_3 = target[:, 2, :, :].unsqueeze(1)
            self.scheduler(self.optimizer, iteration, epoch, self.best_pred)
            self.optimizer.zero_grad()
            if self.args.model == 'multiPI_TransBTS':
                output, output_WT, output_TC,output_ET = self.model(image)
            ...
            if self.args.model == 'multiPI_TransBTS':
                ...
                output1 = torch.sigmoid(output_WT)
                output2 = torch.sigmoid(output_TC)
                output3 = torch.sigmoid(output_ET)
                dice_loss_1 = self.criterion1(output1, target_1)
                loss_1 = self.criterion2(output1, target_1)
                dice_loss_2 = self.criterion1(output2, target_2)
                loss_2 = self.criterion2(output2, target_2)
                dice_loss_3 = self.criterion1(output3, target_3)
                loss_3 = self.criterion2(output3, target_3)
                loss1 = dice_loss_1 + loss_1
                loss2 = dice_loss_2 + loss_2
                loss3 = dice_loss_3 + loss_3
                loss = 0.3*loss1 + 0.3*loss2 + 0.3*loss3
            else:
    ...
    def validation(self, epoch):
        ...
        for iteration, sample in enumerate(tbar):
            ...
            with torch.no_grad():
                if self.args.model == 'multiPI_TransBTS':
                    output, output_m1, output_m2, output_m3 = self.model(image)
                elif self.args.modalities == 'all':
                    output = self.model(image)
                else:
                    raise RuntimeError("modalities error, chossen{}".format(self.args.modalities))
